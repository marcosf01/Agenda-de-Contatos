...
begin
  if dmContatos.FDTable1.FindKey([edtBarraDePesquisa.Text]) then
    ...
    Application.MessageBox('O contato não foi encontrado!', 'Atenção', MB_OK + MB_ICONWARNING);
...
begin
  ...
  begin
    edtID.SetFocus;
    Application.MessageBox('O campo "ID" não pode ficar vazio!', 'Atenção', MB_OK + MB_ICONWARNING);
    Abort;
  end;
  ...
  begin
    edtNome.SetFocus;
    Application.MessageBox('O campo "Nome" não pode ficar vazio!', 'Atenção', MB_OK + MB_ICONWARNING);
    Abort;
  end;
  ...
  begin
    ...
    Application.MessageBox('O campo "Telefone" não pode ficar vazio!', 'Atenção', MB_OK + MB_ICONWARNING);
    Abort;
  end;
  ...
  begin
    edtEmail.SetFocus;
    Application.MessageBox('O campo "E-mail" não pode ficar vazio!', 'Atenção', MB_OK + MB_ICONWARNING);
    Abort;
  end;
  ...
  begin
    ...
    Application.MessageBox('O campo "Observações" não pode ficar vazio!', 'Atenção', MB_OK + MB_ICONWARNING);
    Abort;
  end;
...
begin
  ...
  if dmContatos.FDTable1.FieldByName('foto').Value = NULL then
    Image1.Picture.LoadFromFile('C:\Users\marco\OneDrive\Área de Trabalho\Delphi - Agenda de Contato\pessoas\img_no_photo.jpg')
  else
